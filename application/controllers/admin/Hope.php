...
class Hope extends Admin_Controller
{
    ...
    public function index()
    {
		...
		if(!$this->session->flashdata('success')) {
			...
			$data['error'] = $this->session->flashdata('error');
				...
				$this->load->view('admin/hope/index', $data);
	...
	public function paginate(){
		...
		$data= $this->hope_model->get_paginated_events($pageNum, $pageSize);
       ...
       $data ='';
	  	if($this->input->post('submit')) {
	 		  ...
	 		  if ($this->form_validation->run() == FALSE){
                ...
                {
                $config['upload_path']          = './uploads/hope/';
              ...
              if(isset($_FILES['image']) && $_FILES['image']['size'][0] > 0){
                	 if ( ! $this->upload->do_multi_upload('image')){
		                       ...
		                       $data['error'] = $error;
			                        ...
			                        for($i=0; $i < sizeof($data); $i++){
			                        	if($i ==0){
			                        		 $image .=   IMAGES_URI.''.'hope/'.$data[$i]['file_name'];
			                        		}else{
			                        			 $image .=  ','.IMAGES_URI.''.'hope/'.$data[$i]['file_name'];
			                        		}
			                        }
			       ...
			       $_POST['image'] = $image;
			      $insertStatus=  $this->hope_model->create_event($this->input->post());
			     if($insertStatus){
	  	...
	  	$data['services'] = $this->hope_model->get_services();
	  		
	  	$this->load->view('admin/templates/header');
		$this->load->view('admin/templates/sidebar');
		$this->load->view('admin/templates/topbar');
		$this->load->view('admin/hope/create',$data);
		$this->load->view('admin/templates/footer');
	}
	//--------------------------------------------------------------------
	...
	public function edit($eventId){
		if($this->input->post('submit') && is_numeric($eventId)) {
              ...
              if(isset($_FILES['image']) && $_FILES['image']['size'][0] > 0){
			       ...
			       $_POST['image'] = $image;
			      $updateStatus=  $this->hope_model->update_event($this->input->post(),$eventId);
			     if($updateStatus){
	...
	$data['services'] = $this->hope_model->get_services();
		...
		if(is_numeric($eventId)){
				$data['event'] = $this->hope_model->get_event_by_id($eventId);
				$this->load->view('admin/templates/header');
				$this->load->view('admin/templates/sidebar');
				$this->load->view('admin/templates/topbar');
				$this->load->view('admin/hope/edit',$data);
				$this->load->view('admin/templates/footer');
		}
	}
		...
		public function delete($eventId){
			 ...
			 $user_role = $this->role_model->find($this->current_user->role_id);
       		 $default_context = ($user_role !== false && isset($user_role->default_context)) ? $user_role->default_context : '';
			$deleteStatus= $this->hope_model->delete_event($eventId);
			  if($deleteStatus){
